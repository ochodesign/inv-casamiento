...
const Countdown = ({ targetDate }) => {
  ...
  <svg className="absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 pointer-events-none" width="420" height="420" viewBox="0 0 420 420" fill="none" xmlns="http://www.w3.org/2000/svg" style={{maxWidth:'100vw',overflow:'visible',zIndex:2}}>
          <circle cx="210" cy="210" r="180" stroke="#bfa16a" strokeWidth="2" fill="none" />
          <circle cx="210" cy="210" r="195" stroke="#bfa16a" strokeWidth="1" fill="none" />
          <circle cx="210" cy="210" r="208" stroke="#bfa16a" strokeWidth="1" fill="none" />
        </svg>
  <div className="relative bg-white rounded-full shadow-xl flex flex-col items-center justify-center w-[340px] h-[340px] max-w-full px-4 py-8 border-0 mx-auto" style={{margin:'0 auto', zIndex:3}}>
          <div className="text-3xl md:text-4xl font-bold text-[#bfa16a] mb-1 font-lora">Faltan</div>
          <div className="w-12 h-1 mx-auto mb-2 border-b-2 border-[#bfa16a] rounded-full"></div>
          <div className="flex justify-center gap-6 text-2xl md:text-3xl font-bold text-[#6b4f27] mb-2">
            <div className="flex flex-col items-center">
              <span>{timeLeft.days}</span>
              <span className="text-[#bfa16a] text-base font-normal mt-1">días</span>
            </div>
            <div className="flex flex-col items-center">
              <span>{timeLeft.hours}</span>
              <span className="text-[#bfa16a] text-base font-normal mt-1">hs</span>
            </div>
            <div className="flex flex-col items-center">
              <span>{timeLeft.minutes}</span>
              <span className="text-[#bfa16a] text-base font-normal mt-1">min</span>
            </div>
            <div className="flex flex-col items-center">
              <span>{timeLeft.seconds}</span>
              <span className="text-[#bfa16a] text-base font-normal mt-1">seg</span>
            </div>
          </div>
          <div className="flex justify-center mt-2">
            <svg width="32" height="32" viewBox="0 0 24 24" fill="#bfa16a" xmlns="http://www.w3.org/2000/svg">
